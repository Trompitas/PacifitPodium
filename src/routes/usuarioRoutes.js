...
router.put("/podium/:id", (req, res) => {
  const { id } = req.params;
  const { nombreCompleto, cantidadRepeticiones, masculinofemenino, nombreUsuario} = req.body;
  usuarioSchema
    .updateOne(
      { _id: id },
      { $set: { nombreCompleto, cantidadRepeticiones, masculinofemenino, nombreUsuario } }
    )
    .then((data) => res.json(data))
    .catch((error) => res.json({ message: error }));
});
